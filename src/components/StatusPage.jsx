...
const Status = (props) => {
    ...
    const feedSearch = () =>{
        if (feedSearchInput.length > 1) {
            ...
            axios.get(`https://gnews.io/api/v4/search?q=${feedSearchInput}&lang=en&max=5&apikey=10&apikey=89bf5943fba1916d39f48ea649942452`)
            .then((output)=>{
    ...
    return (
        <div className="stat-overall" onClick={(e)=>{closeTopOption(e)}}>
            <h1>Status</h1>
                    ...
                    <div className="shrink">
                        ...
                        </div>
                        {
                            props.sortUsers.map((TotalResult,index)=>{
                                ...
                                const user = Object.keys(TotalResult)[0]
                                const getCurrentUser = props.allFriends?.filter(friend=> friend.UserName == user)
                                const img = getCurrentUser[0]?.profilePic
                                return(
                                    <div className="user-status" key={index} onClick={activePreview?null:preview} id={`userSatus${index}`}>
                                        ...
                                        <img src={img} alt="" className='profile-mini'/>
                                        <div className='status-upload-parent'>
                                            ...
                                            <p className='userName'>{mapItem.length > 0? mapItem[0].userName : null}</p>
                                                {
                                                    mapItem.map((status,statusIndex)=>{
                                                        return(
