...
import { onChildAdded, ref, query, orderByChild, limitToLast, onValue, get, onChildChanged, set, update } from "firebase/database"
import axios from 'axios'
import logo from "../images/clinic-02.svg"
import { BrowserRouter as Router, Routes, Route, Navigate, useNavigate } from 'react-router-dom';
import Settings from './Settings.jsx'
import ChatDisplayMini from './ChatDisplayMini.jsx'


const SideComponents = (props) => {
    const navigate = useNavigate()
    let userName
    const [miniChatInfo, setMiniChatInfo] = useState()
    const [miniChatArray, setMiniChatArray] = useState([])
    const [miniChatFriendDetails, setMiniChatFriendDetail] = useState()
    ...
    const [changeSection, setChangeSection] = useState("Chats")
    ...
    useEffect(() => {
        const userNameLoc = JSON.parse(localStorage.getItem("TilChat"))
        const newFreinds = onValue(ref(db, `Users/${userNameLoc ? userNameLoc?.UserName : null}/newFriends`), (data) => {
            if (data.val()) {
                let holdData
                holdData = data.val()
                holdData?.map((data, index) => {
    ...
    useEffect(() => {
        ...
        const statusList = onValue(ref(db, `Users/${userName}/StatusPending/list`), (output) => {
            if (output.exists()) {
    ...
    useEffect(() => {
        if (props.sectionOrder) {

            if (props.sectionOrder == "ai") {
                ...
                setChangeSection(props.sectionOrder)
            }
            if (!props.openMsg) {
                props.setReturnStatement("Done")
            }
            props.setSectionOrder(null)
        }
    }, [props.sectionOrder])
    ...
    if (changeSection == "Updates") {
        ...
        return (
            <div className='side-components-parent' style={{ display: "flex" }}>
                ...
                <div className='chats-Parent-overall'>
                    {miniShow ? <ChatDisplayMini miniChatFriendDetails={miniChatFriendDetails} miniChatArray={miniChatArray} setMiniChatArray={setMiniChatArray} miniChatInfo={miniChatInfo} setMiniChatInfo={setMiniChatInfo} setMiniShow={setMiniShow} chatBlocked={props.chatBlocked} setChatBlocked={props.setChatBlocked} archivedArray={props.archivedArray} setArchivedArray={props.setArchivedArray} friendsWallpaper={props.friendsWallpaper} currentWallpaper={props.currentWallpaper} chatArray={props.chatArray} setChatArray={props.setChatArray} forwardChatInfo={props.forwardChatInfo} deviceUserAgent={props.deviceUserAgent} otherDevices={props.otherDevices} chatState={props.chatState} chatInfo={props.chatInfo} chatFriendDetail={props.chatFriendDetail} /> : null}
                    <Chats setCreateGroup={props.setCreateGroup} setAskMsg={props.setAskMsg} setReturnStatement={props.setReturnStatement} setOpenMsg={props.setOpenMsg} openMsg={props.openMsg} setMiniChatFriendDetail={setMiniChatFriendDetail} miniChatArray={miniChatArray} setMiniChatArray={setMiniChatArray} miniChatInfo={miniChatInfo} setMiniChatInfo={setMiniChatInfo} miniShow={miniShow} setMiniShow={setMiniShow} archivedArray={props.archivedArray} chatArray={props.chatArray} setChatArray={props.setChatArray} triggerForward={props.triggerForward} setTriggerSend={props.setTriggerSend} forwardChatInfo={props.forwardChatInfo} forwardArray={props.forwardArray} setChangeSection={setChangeSection} setViewState={props.setViewState} setChatSearchFilter={setChatSearchFilter} chatSearchFilter={chatSearchFilter} setChatView={props.setChatView} setChatState={props.setChatState} setChatInfo={props.setChatInfo} chatInfo={props.chatInfo} setChatFriendDetail={props.setChatFriendDetail} mutualRender={props.mutualRender} setMutualRender={props.setMutualRender} userCredentials={props.userCredentials} />
                </div>
            </div>
        )
    }
        ...
        return (
            <div className='live-side side-components-parent' style={{ display: "flex" }}>
                {!props.isAwake ?
                    (<h1 onClick={startAI}>T</h1>)
                    : null}
                <Sider userCredentials={props.userCredentials} setIsAwake={props.setIsAwake} setChatState={props.setChatState} changeSection={changeSection} setChangeSection={setChangeSection} setViewState={props.setViewState} ViewState={props.ViewState} fetchVideo={fetchVideo} setChatView={props.setChatView} />
        ...
        return (
            <div className='side-components-parent' style={{ display: "flex" }}>
                ...
                <Sider userCredentials={props.userCredentials} setIsAwake={props.setIsAwake} setChatState={props.setChatState} changeSection={changeSection} setChangeSection={setChangeSection} setViewState={props.setViewState} ViewState={props.ViewState} fetchVideo={fetchVideo} setChatView={props.setChatView} />
                <div className='chats-Parent-overall'>
                    <Settings mutualRender={props.mutualRender} setMutualRender={props.setMutualRender} setWallPaperArray={props.setWallPaperArray} wallPaperArray={props.wallPaperArray} currentWallpaper={props.currentWallpaper} setCurrentWallpaper={props.setCurrentWallpaper} userCredentials={props.userCredentials} setChatState={props.setChatState} fetchVideo={fetchVideo} videoItems={videoItems} setVideoSearch={setVideoSearch} videoSearch={videoSearch} setViewState={props.setViewState} ViewState={props.ViewState} setIframeLink={props.setIframeLink} setChatView={props.setChatView} />
                </div>
            </div>
        )
    }
