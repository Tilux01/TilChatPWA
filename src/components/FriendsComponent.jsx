...
const FriendsComponent = (props) => {
    ...
    const searchFriends = async () => {
        ...
        try {
            ...
            const [nameSnapshot, usernameSnapshot, mailSnapshot] = await Promise.all([
                get(nameQuery),
                get(usernameQuery),
                get(mailQuery)
            ]);
            ...
            if (mailSnapshot.exists()) {
                Object.assign(combinedResults, mailSnapshot.val())
            }
    ...
    const addFriend = (params, e) => {
        ...
        document.querySelector(`#${parentId} .cancel-btn`).style.display = "block"
        get(ref(db, `Users/${params.UserName}/friendsArray`))
            .then((data) => {
                ...
                sendToNodeServer(params.UserName, "TIlChat", `${userCredentials.UserName} sent you a  friend request`)
            })
    }
                            ...
                            .finally(() => {
                                ...
                                get(ref(db, `Users/${output.UserName}/newFriends`))
                                    .then((data) => {
                                        ...
                                        sendToNodeServer(output.UserName, "TIlChat", `${userCredentials.UserName} confirmed your friend request`)
                                    })
                            })
                    })
            })
    ...
    return (
        <div className="friends-overall" onClick={(e) => { closeTopOption(e) }}>
            <h1>Friends</h1>
            ...
            <div className="friends-parent">
                                    ...
                                    return (
                                        <div id={`parent${index}`} className="friends" key={`nonfriend-${index}`}>
                                            ...
                                            <div>
                                                ...
                                                <small style={{ color: "white", position: "absolute", bottom: "10px" }}>@{output.UserName}</small>
                                                <div className="Btn">
                                                    <button onClick={(e) => addFriend(output, e)} className="add-btn" >Add Friend</button>
                                                    <button style={{ display: "none" }} className='message-btn cancel-btn'>Cancel Request</button>
                                                </div>
                                            </div>
                                        </div>
                                    );
                ...
                {props.mutualRender != [] ? <h3 style={{ marginTop: "15px", color: "white", fontSize: "24px" }}>Friends</h3> : null}
                {
